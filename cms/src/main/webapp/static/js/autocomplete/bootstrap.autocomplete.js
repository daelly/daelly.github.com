 ...
 * ================================= 
 * DreamLu 根据需求再次微调， http://www.dreamlu.net/
 */
  ...
  , addtag: function () {
      ...
      if (!this.check(value)) {
        $(".tag-list").append('<li class="attached"><span class="tag">' + value + '</span><a class="icon-remove" title="删除" href="#"></a><input name="tags" value="' + id + '" type="hidden"></li>')
        this.$element.val('')
        return this.hide()
      }
    }
  ...
  , check: function (value) {
      var temp = false
      $(".tag-list .tag").map(function(i, v) {
        var text = $(v).text()
        if ( text == value ) {
          $(v).fadeOut("fast").fadeIn("fast")
          temp = true;
        }
      })
      return temp
